...
import qrCodeDeleteAction from "@/actions/qrcode-delete-action";

export default function QrCodeDeleteModal({
  qrCode,
}: {
  qrCode: PrismaQrCode;
}) {
  ...
  const { execute, result, status } = useAction(qrCodeDeleteAction, {
    onSuccess: () => {
      setOpen(false);
      router.refresh();
    },
